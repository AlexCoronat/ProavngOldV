...
public class NullConverter {
	...
	/**
	 * servidorToString recibe un objeto de tipo Visitante y a su vez se accede a un
	 * objeto lector contenido dentro de Visitante y por medio de una condición, se
	 * evalua que el objeto no sea nulo, de ser así su valor nulo será reemplazado
	 ...
	 *         si el objeto es nulo.
	 */
	public static String servidorToString(Visitante v) {
		return v.getLector().getServidor() == null ? "" : v.getLector().getServidor();
	}
	public static String pasaporteToString(Visitante v) {
		return v.isPasaporte() == true ? "true" : "false";
	}
	public static int convert(int input) {
		String dato = Integer.toString(input);
		return dato == null ? 0 : input;
	}
